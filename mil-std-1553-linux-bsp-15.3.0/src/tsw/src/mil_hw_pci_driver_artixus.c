...
/*! \file mil_hw_pci_artixus.c
    This file contains hw layer implementations for ArtixUS without ASP.
...
AiInt16 mil_hw_artixus_internal_tcp_command(TY_API_DEV *  p_api_dev,
    AiUInt32 ulCommand,
...
{
    p_api_dev->IoRAMBase[ARTIXUS_REG_NOVRAM_BASE_OFFSET + ulOffset/4] = ulValue;
...
{
    *pulValue = p_api_dev->IoRAMBase[ARTIXUS_REG_NOVRAM_BASE_OFFSET + ulOffset/4];
...
AiUInt32 mil_hw_pci_driver_artixus_novram_finalize(TY_API_DEV *p_api_dev)
{
    ...
    wRetVal = mil_hw_artixus_internal_tcp_command(p_api_dev, ARTIXUS_MON_CMD_NOVRAM_FLUSH, 0, 0, 1, &ulOutput);

    if ((wRetVal  == ARTIXUS_MON_OK)
     && (ulOutput != ARTIXUS_MON_PASS))
    {
        wRetVal = ARTIXUS_MON_ERR_STATUS;
    }
    ...
    AiUInt32 ulOutputCount, AiUInt32 ulOutput[])
{
    ...
    for (i = 0; i<ulInputCount; i++)
        p_api_dev->IoRAMBase[ARTIXUS_REG_TCP_INPUT_0 + i] = ulInput[i];

    /* execute command */
    ulCommand = ulCommand << 24;

    p_api_dev->IoRAMBase[ARTIXUS_REG_TCP_CONTROL] = ulCommand;
    ...
    for (i = 0; i<10000; i++)
    {
        ulValue = p_api_dev->IoRAMBase[ARTIXUS_REG_TCP_STATUS];

        if (((ulValue & ARTIXUS_MON_STATUS_MASK) == ARTIXUS_MON_STATUS_FREE)
            && ((ulValue & ARTIXUS_MON_STATUS_CMD) == ulCommand))
            /* finished */
            break;
    ...
    if (i == 10000)
        /* timeout occured */
        return ARTIXUS_MON_ERR_TIMEOUT;

    /* copy return data */

    if (ulOutputCount > 0)
        for (i = 0; i<ulOutputCount; i++)
            ulOutput[i] = p_api_dev->IoRAMBase[ARTIXUS_REG_TCP_OUTPUT_0 + i];

    /* -- execute command 0 to reset the status --- */
    p_api_dev->IoRAMBase[ARTIXUS_REG_TCP_CONTROL] = ulCommandZero;

    return ARTIXUS_MON_OK;
}
...
L_WORD mil_hw_pci_driver_artixus_irig_set_external(TY_API_DEV *p_api_dev, BYTE con)
{
    ...
    if (con & 0x01)
    {
        value = p_api_dev->IoRAMBase[ARTIXUS_REG_TIME_SOURCE_CONTROL];
        value |= 0x00000100L; /* Bit 8=1 is irig external*/
        p_api_dev->IoRAMBase[ARTIXUS_REG_TIME_SOURCE_CONTROL] = value;
    }
    else
    {
        value = p_api_dev->IoRAMBase[ARTIXUS_REG_TIME_SOURCE_CONTROL];
...
/*!
    ...
    \return ackfl      BYTE             Instruction acknowledge type.
*/
L_WORD mil_hw_pci_driver_artixus_irig_set( TY_API_DEV * p_api_dev, 
                        L_WORD         day, 
                        L_WORD         hour, 
                        L_WORD         min, 
                        L_WORD         sec,
                        L_WORD         us )
{
    ...
    */
    ...
    irig_status = p_api_dev->IoRAMBase[ARTIXUS_REG_TIME_SOURCE_CONTROL];

    if (irig_status & ARTIXUS_REG_TIME_SOURCE_CONTROL_BIT_INSTANT)
    {
        /* This will only set the the on-board time register which is decoupled from the irig encoder/decoder */
        p_api_dev->IoRAMBase[ARTIXUS_REG_TT_HIGH] = ulInput[0];
        p_api_dev->IoRAMBase[ARTIXUS_REG_TT_LOW]  = min << 26 | sec << 20 | us;
        wbflush();
    }

    /* -- set irig encoder register via TCP-FW command --- */
    wRetVal = mil_hw_artixus_internal_tcp_command(p_api_dev, ARTIXUS_MON_CMD_IRIG_SET, 3, ulInput, 0, 0);
...
L_WORD mil_hw_pci_driver_artixus_irig_get_status(TY_API_DEV *p_api_dev, AiUInt32 * irig_sync, AiUInt32 * irig_src)
{
    ...
    *irig_src    = (irig_status >> 8) & 0x1;
    ...
    irig_status = p_api_dev->IoRAMBase[ARTIXUS_REG_TT_STATUS];
...
/*!
    ...
    \return ackfl      BYTE             Instruction acknowledge type.
*/
L_WORD mil_hw_pci_driver_artixus_irig_get_time(TY_API_DEV * p_api_dev,
                           L_WORD     * sth, 
                           L_WORD     * stl )
{
    *sth = p_api_dev->IoRAMBase[ARTIXUS_REG_TT_HIGH];
    *stl = p_api_dev->IoRAMBase[ARTIXUS_REG_TT_LOW];
...
L_WORD mil_hw_pci_driver_artixus_irig_set_capabilities(TY_API_DEV* p_api_dev)
{
    ...
    if (irig_status & ARTIXUS_REG_TIME_SOURCE_CONTROL_BIT_OUT_SIGNAL)
...
AiReturn mil_hw_pci_driver_artixus_discretes_init(TY_API_DEV *p_api_dev, AiUInt32 new_config)
{
    ...
    current_config = BSWAP32(p_api_dev->IoRAMBase[ARTIXUS_REG_GPIO_CONFIG]);

    current_config &= ~0xFF; /* Clear bits 0-7 */
    new_config &= 0xFF;  /* Only bits 0-7 are valid */
    current_config |= new_config;

    p_api_dev->IoRAMBase[ARTIXUS_REG_GPIO_CONFIG] = BSWAP32(current_config);
...
AiReturn mil_hw_pci_driver_artixus_discretes_write(TY_API_DEV *p_api_dev, AiUInt32 Mask, AiUInt32 Value)
{
    ...
    ul_Tmp = p_api_dev->IoRAMBase[ARTIXUS_REG_GPIO_OUTPUT];
    ul_Tmp &= (Value | ~Mask); /* Set all related bits to low  */
    ul_Tmp |= (Value &  Mask); /* Set all related bits to high */
    p_api_dev->IoRAMBase[ARTIXUS_REG_GPIO_OUTPUT] = ul_Tmp;
...
{
    *Value = p_api_dev->IoRAMBase[ARTIXUS_REG_GPIO_INPUT];
...
enum MIL_FW mil_hw_pci_driver_artixus_get_fw_type(TY_API_DEV * p_api_dev, short ulBiu)
{
    if (API_DEVICE_MODE_IS_EMBEDDED(p_api_dev->drv_mode[ulBiu]))
        return MIL_FW_EBD_ARTIX7;   // AXCX1553 (Embedeed has the same defines as AMEE1553)
    else
        return MIL_FW_STD_ARTIX7;   // AXCX1553 (Test and Simulation has the same defines as AMEE1553-TS)
}
...
{
    AiUInt16 wRetVal   = 0;
    AiUInt32 version[2]   = {0,0};
    AiUInt32 git_hash     = 0;

    wRetVal = mil_hw_artixus_internal_tcp_command(p_api_dev, ARTIXUS_MON_CMD_MYMON_VER, 0, 0, 2, version);

    if (wRetVal  == ARTIXUS_MON_OK)
        wRetVal = ARTIXUS_MON_ERR_STATUS;

    pxVersionValue->ul_MajorVer = BCD2BIN_B(EXTRACT_BITS(version[0], 24, 0xFF));
    pxVersionValue->ul_MinorVer = BCD2BIN_B(EXTRACT_BITS(version[0], 16, 0xFF));
    pxVersionValue->ul_PatchVersion = 0;
    pxVersionValue->ul_BuildNr = BCD2BIN_W(EXTRACT_BITS(version[0], 0, 0xFFFF));
    git_hash = version[1];
    ...
    if (git_hash)
    {
        AI_TSW_OS_SPRINTF(pxVersionString->ac_FullVersion, AI_VERSION_STRINGL, "%d.%d.%d-g%x",
            ...
            pxVersionValue->ul_BuildNr, git_hash);
    } 
    else 
    {
...
L_WORD mil_hw_pci_driver_artixus_version_mlca(TY_API_DEV *p_api_dev, TY_E_VERSION_ID eId, TY_API_VER_INFO_VALUES * pxVersionValue, TY_API_VER_INFO_STRINGS * pxVersionString)
{
    ...
    version = p_api_dev->IoRAMBase[ARTIXUS_REG_PL_VERSION];
...
L_WORD mil_hw_pci_driver_artixus_version_monitor(TY_API_DEV *p_api_dev, TY_E_VERSION_ID eId, TY_API_VER_INFO_VALUES * pxVersionValue, TY_API_VER_INFO_STRINGS * pxVersionString)
{
    ...
    ret = mil_hw_pci_driver_artixus_version_tcp(p_api_dev, eId, pxVersionValue, pxVersionString);
...
{
    p_api_dev->IoRAMBase[ARTIXUS_REG_TRIGGER_DIGITAL_LOOP] = con;
    return 0;
}
...
{
    *con = p_api_dev->IoRAMBase[ARTIXUS_REG_TRIGGER_DIGITAL_LOOP];
    return 0;
}
